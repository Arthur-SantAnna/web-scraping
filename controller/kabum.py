...
class Kabum:
    ...
    def _fetch_product_info(self, url):
        ...
        title = self._extract_text(soup, 'h1', {'class': 'product-title'}, "título", url)
        price = self._extract_price(soup, 'span', {'class': 'final-price'}, url)
    ...
    def _extract_price(self, soup, tag, attrs, url):
        ...
        try:
            # Limpeza do preço, considerando possíveis formatações da KaBuM
            price_str = (
                price_str.replace('R$', '')
                         .replace('.', '')
                         .replace(',', '.')
